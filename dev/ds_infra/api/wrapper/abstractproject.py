...
class AbstractProject(abc.ABC):
    ...
    def __init__(self, query, conn, test=False,workflow_name=None):
        ...
        if workflow_name == None:
            ...
            if workflow_name == "subjectline":
                self.request = ds_infra.api.rest.common.request.SLScoreRequest(query,test=test)
            if workflow_name == "intelligentswitch":
                self.request = ds_infra.api.rest.common.request.ISwitchScoreRequest(query,test=test)
            if workflow_name == "gen_sl":
                self.request = ds_infra.api.rest.common.request.SLGenRequest(query,test=test)
            if workflow_name == "patient_classification":
                self.request = ds_infra.api.rest.common.request.PatientClassificationRequest(query,test=test)
